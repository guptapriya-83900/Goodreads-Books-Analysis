...
if not os.path.exists('best-books-ever_data.csv'):
  ...
  book_data.to_csv('best-books-ever_data.csv')
...
for i in range(len(book_data), len(books)):
    ...
    try:
        ...
        if i%save_every==0:
            book_data.append(pd.DataFrame.from_dict(book_rows)).to_csv('best-books-ever_data.csv', index=False)
            book_rows=[]
    except:
        book_data.append(pd.DataFrame.from_dict(book_rows)).to_csv('best-books-ever_data.csv', index=False)
        book_rows=[]