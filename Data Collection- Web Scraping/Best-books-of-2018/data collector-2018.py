#Data Collector: To collect data on each book in the file books.csv
...
if not os.path.exists('best-books-of-2018_data.csv'):
  ...
  book_data.to_csv('best-books-of-2018_data.csv')

books=pd.read_csv('books.csv')
book_data=pd.read_csv('best-books-of-2018_data.csv')
...
for i in range(len(book_data), len(books)):
    ...
    try:
        ...
        if i%save_every==0:
            book_data.append(pd.DataFrame.from_dict(book_rows)).to_csv('best-books-of-2018_data.csv', index=False)
            book_rows=[]
    except:
        book_data.append(pd.DataFrame.from_dict(book_rows)).to_csv('best-books-of-2018_data.csv', index=False)
        book_rows=[]